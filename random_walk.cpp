...
void decompose_domain(int domain_size, int world_rank,
    ...
    if (world_size > domain_size) {
        // We just assume that domain size is greater than the world size.
...
void
initialize_walkers(int num_walkers_per_proc, int max_walk_size, int subdomain_start, vector<Walker> *incoming_walkers) {
    Walker walker;
...
void send_outgoing_walkers(vector<Walker>* outgoing_walkers, int world_rank, int world_size) {
    // Send the data as an array of MPI_BYTE to the next process. The last process
    // sends to process zero.
...
void receive_incoming_walkers(vector<Walker>* incoming_walkers, int world_rank, int world_size) {
    MPI_Status status;
    // Receive from the process before you. If you are process zero, receive from the last
    // process
    ...
    MPI_Recv((void*)incoming_walkers->data(), incoming_walker_size, MPI_BYTE, incoming_rank,
...
int main(int argc, char** argv) {
    ...
    cout << "Process" << world_rank << " initiated " << num_walkers_per_proc << " walkers in subdomain "
